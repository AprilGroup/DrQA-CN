...
# mbaqa not found without the them
import sys
sys.path.append('../../')
...
def count(ngram, hash_size, doc_id):
    ...
    # save unigrams and bigrams respectively for debug
    unigrams = [gram for gram in ngrams if gram.find(' ') == -1]
    bigrams = [gram for gram in ngrams if gram.find(' ') != -1]

    # Hash ngrams and count occurences
    # counts = Counter([retriever.utils.hash(gram, hash_size) for gram in ngrams])

    # Hash ngrams and save gram to hash map for debug
    hashes = []
    # gram2hash = {gram: hashcode for hashcode, gram in hash2gram.items()}
    hash2gram = {}
    for gram in ngrams:
        # compute hashcode
        hashcode = retriever.utils.hash(gram, hash_size)

        # if hashcode not in hash2gram:
        #     hash2gram[hashcode] = set()
        # hash2gram[hashcode].add(gram)
    ...
    return row, col, data, unigrams, bigrams, hash2gram
...
def get_count_matrix(args, db, db_opts):
    ...
    for i, batch in enumerate(batches):
        ...
        for b_row, b_col, b_data, b_unigrams, b_bigrams, b_hash2gram in workers.imap_unordered(_count, batch):
            row.extend(b_row)
            col.extend(b_col)
            data.extend(b_data)
...
def get_title_tfidf_matrix(args, db, db_opts, doc_freqs):
    ...
    tfidf_matrix = sp.csr_matrix(
        (datas, (rows, cols)), shape=(args.hash_size, len(doc_ids))
    )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--db-path', type=str, default='../../data/db/mba.db',
                        help='Path to sqlite db holding document texts')
    parser.add_argument('--out-dir', type=str, default='../../data/retriever/',
    ...
    parser.add_argument('--num-docs', type=int, default=80000,
                        help='select first number of docs for testing.')
    parser.add_argument('--tokenizer', type=str, default='ltp',
                        help=("String option specifying tokenizer type to use "
                              "(e.g. 'corenlp')"))
    parser.add_argument('--num-workers', type=int, default=4,
                        help='Number of CPU processes (for tokenizing, etc)')
    parser.add_argument('--matrix-type', type=str, default='csr',
                        help='csc or csr matrix')
    args = parser.parse_args()

    logging.info('Counting words...')
    count_matrix, doc_dict, hash_gram = get_count_matrix(
    ...
    title_tfidf = get_title_tfidf_matrix(
        args, 'sqlite', {'db_path': args.db_path}, freqs
    )

    basename = os.path.splitext(os.path.basename(args.db_path))[0]
    basename += ('-tfidf-ngram=%d-hash=%d-tokenizer=%s-numdocs=%s' %
                 (args.ngram, args.hash_size, args.tokenizer, count_matrix.shape[1]))
    ...
    metadata = {
        ...
        'ngram': args.ngram,
        'doc_dict': doc_dict,
        'unigrams': hash_gram[0],
        'bigrams': hash_gram[1],
        'hash2gram': hash_gram[2],
