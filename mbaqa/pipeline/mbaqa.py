...
class DrQA(object):
    ...
    def process_batch(self, queries, candidates=None, top_n=1, n_docs=5,
                      return_context=False, after_process_on=True):
        ...
        if len(queries) == 1:
            ranked = [self.ranker.closest_docs_by_content_and_title(queries[0], k=n_docs)]
        else:
        ...
        # logger.info('Retrive text from database in %.4f (s)' %
        #             (time.time() - t1))
        ...
        # logger.info('Split and flatten documents in %.4f (s)' %
        #             (time.time() - t2))
        ...
        # logger.info('Tokenize question and documents in %.4f (s)' %
        #             (time.time() - t3))
        ...
        # logger.info('decode argmax start/end indicesin %.4f (s)' %
        #             (time.time() - t4))
        ...
        for queue in queues:
            ...
            while len(queue) > 0:
                ...
                if after_process_on:
                    # logger.info('raw answer: ' + prediction['span'])
                    prediction['span'] = self.after_process(prediction['span'], prediction['context']['text'])
                    # logger.info('after ans : ' + prediction['span'])
                    prediction['context']['start'] = prediction['context']['text'].find(prediction['span'])
                    prediction['context']['end'] = prediction['context']['start'] + len(prediction['span'])
                predictions.append(prediction)
            all_predictions.append(predictions[-1::-1])



        # logger.info('Arrange prediction data %.4f (s)' %
        #             (time.time() - t5))
    ...
    def after_process(answer_span, context):
        ...
        # logger.info('remove head:' + answer_span)
        ...
        # logger.info('add head:   ' + answer_span)
        ...
        if last_period:  # period found
            ...
            # logger.info('remove tail:' + answer_span)
        else:
            # or extend the answer span to the nearest period
            search_span = context[answer_start + len(answer_span):]
            logger.info('search span:' + search_span)
            first_period_after_answer = regex.search(r'。', search_span)
            if first_period_after_answer:
                answer_span = context[answer_start: first_period_after_answer.start() + answer_start + len(answer_span) + 1]
            # logger.info('add tail:   ' + answer_span)
